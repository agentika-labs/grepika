...
/// Extracts file structure via tree-sitter (see [`crate::services::ast`]).
/// Returns an empty list for unsupported languages or unparseable source.
fn extract_symbols(content: &str, file_type: &str) -> Vec<Symbol> {
    crate::services::ast::extract(content, file_type)
        .into_iter()
        .map(|s| Symbol {
            name: s.name,
            kind: s.kind,
