...
1. **Search for the error/keyword**
   - Use `mcp__agentika-grep__search` to find matches for the error message or keywords
   - Try both exact matches and semantic variations

2. **Get context around matches**
   - Use `mcp__agentika-grep__context` to see surrounding code for each match
   - Identify which matches are the actual error origin vs error handling

3. **Find references to key functions**
   - Use `mcp__agentika-grep__refs` to trace function calls
...
5. **Extract file structure**
   - Use `mcp__agentika-grep__outline` on key files to understand their shape
